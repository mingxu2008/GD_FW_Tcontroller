...
{      			    
    ...
    for(t=0;t<15;t++)
		{
				OLED_WR_Byte(Symbol[2*no][t],OLED_DATA);
				adder+=1;
     }	
		OLED_Set_Pos(x,y+1);	
    for(t=0;t<15;t++)
			{	
				OLED_WR_Byte(Symbol[2*no+1][t],OLED_DATA);
				adder+=1;
      }					
}
...
{ 	
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2|GPIO_Pin_3|GPIO_Pin_4|GPIO_Pin_5|GPIO_Pin_7;	 //PA2 PA3 PA4 PA5 PA7  
  ...
  OLED_RST_Set();
	...
	OLED_WR_Byte(0xd3,OLED_CMD); // Set SEG Output Current Brightness 0xd3
	...
	OLED_WR_Byte(0x7f,OLED_CMD);//Set VCOM Deselect Level
	//OLED_WR_Byte(0x00,OLED_DATA);//Set VCOM Deselect Level
